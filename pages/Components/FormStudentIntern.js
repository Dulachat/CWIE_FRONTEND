...
export default function FormStudentIntern() {
  ...
  useEffect(() => {
    if (item.id !== undefined) {
      axiosInstance
        .get("assessment/getOneDetail/" + item.id)
        .then((res) => {
        ...
        {item?.waitings_status === "1" && (
          <div className="w-full mt-2 ">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
              รายชื่อพี่เลี้ยง
              {detailHead?.evaluator_2 | (detailHead?.evaluator_2 === null) &&
                detailHead?.data && (
                  <Form.Item
                    name={"evaluator2_id"}
                    initialValue={detailHead?.evaluator_2}
                    rules={[{ required: true, message: "กรุณาเลือกพี่เลี้ยง" }]}
                  >
                    <Select size={size}>
                      {detailHead?.data?.map((data) => (
                        <Select.Option
                          key={data.id}
                          value={data.id}
                          initialValue={`${
                        ...
                        >
                          <p>{data.fname_TH + " " + data.lname_TH}</p>
                        </Select.Option>
                      ))}
                    </Select>
                  </Form.Item>
